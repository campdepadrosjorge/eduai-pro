...
function userGen(type, topic, diff, extra, subject) {
  ...
  const m = {
    ...
    actividad:   `Actividad didáctica sobre: "${topic}" | Dificultad: ${diff}\n\nIncluí: título, objetivos, duración, materiales, desarrollo completo (inicio/desarrollo/cierre), consignas exactas para alumnos, criterios de evaluación, variantes.${topic.toLowerCase().includes("micro") || topic.toLowerCase().includes("makecode") || topic.toLowerCase().includes("bloque") || topic.toLowerCase().includes("programa") || subject?.name?.toLowerCase().includes("micro") || curSubj?.name?.toLowerCase().includes("robótica") || curSubj?.name?.toLowerCase().includes("robotica") || curSubj?.name?.toLowerCase().includes("programación") || curSubj?.name?.toLowerCase().includes("programacion") || curSubj?.name?.toLowerCase().includes("tecnología") || curSubj?.name?.toLowerCase().includes("tecnologia") || (genExtra && (genExtra.toLowerCase().includes("makecode") || genExtra.toLowerCase().includes("micro"))) ? "\n\nIMPORTANTE: Al final de la actividad incluí una sección '## Código MakeCode' con el código JavaScript completo para micro:bit que los alumnos deben programar. El código debe estar en un bloque de código con triple backtick javascript y ser funcional en MakeCode." : ""}${e}`,
...
Promise.all([
  ...
  async function generate() {
    ...
    try {
      ...
      const usr = userGen(genType, genTopic, genDiff, genExtra, curSubj);
