// api/generate.js
// Vercel Serverless Function — proxy seguro hacia Anthropic API

export default async function handler(req, res) {
  if (req.method !== "POST") return res.status(405).json({ error: "Method not allowed" });

  const { system, messages, maxTokens = 4000 } = req.body;
  ...
  try {
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        system: system || "Sos un asistente educativo experto.",
        messages,
      }),
    });
